...
export class ReplayTarget implements ExecutionTarget {
  ...
  public readonly type: ExecutionTargetType = 'replay';
  private lifecycle: ExecutionLifecycle = new ExecutionLifecycle();

  constructor(id: string = 'replay-default', name: string = 'Market Replay Target') {
    this.id = id;
    this.name = name;
  }
  ...
  public acknowledge(request: ExecutionRequest): ExecutionAcknowledgementData {
    return ExecutionAcknowledgement.accept(request, this.id, 'Replay target acknowledged request');
  }
