...
export class BrokerTarget implements ExecutionTarget {
  public readonly id: string;
  public readonly name: string;
  public readonly type: ExecutionTargetType = 'broker';
  private lifecycle: ExecutionLifecycle = new ExecutionLifecycle();

  constructor(id: string = 'broker-default', name: string = 'Live Broker Target') {
    this.id = id;
    this.name = name;
  }
  ...
  public acknowledge(request: ExecutionRequest): ExecutionAcknowledgementData {
    return ExecutionAcknowledgement.accept(request, this.id, 'Broker target acknowledged request');
  }
